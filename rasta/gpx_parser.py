...
class GpxParser():
    ...
    def __init__(self, gpx_file_name, calculate_distance=False):
        ...
        if calculate_distance:
            ...
            if not self.data["time"].isnull().values.any():
                ...
                print("Data don't have timestamps -> No speed calculation!")
    ...
    def get_distance_from_df(self):
        ...
        for i in range(0, len(self.data) - 1):
            self.data.iloc[i+1, 4] = self.calculate_distance(
                    self.data["lat"][i],
                    self.data["lon"][i],
                    self.data["lat"][i+1],
                    self.data["lon"][i+1])
    ...
    def get_speed(self):
        for i in range(0, len(self.data) - 1):
            self.data.iloc[i+1, 5] = (3600*self.data.iloc[i+1, 4] /
                          (self.data.iloc[i+1, 3]-self.data.iloc[i, 3])
                          .seconds)

...
for k in range(0, len(nav_data)):
    ...
    if ((nav_data["type"][k] == "PrivateTransportManeuverType") and (nav_data["distance"][k] > 0.5)):
        _, bike_stn_loc = find_nearby_bike_station(12.973132, 77.601479,
...
print("Total walking distance", np.sum(
    gdf_123["distance"][gdf_123["type"] == "Walk"]), "km")
print("You can save ", np.sum(gdf_123["distance"][gdf_123["type"] != "Uber"])*130,
...
print("Total walking distance", np.sum(
    gdf_123_o1["distance"][gdf_123_o1["type"] == "Walk"]), "km")
print("You can save ", np.sum(gdf_123_o1["distance"][gdf_123_o1["type"] != "Uber"])*130,
      "gram of CO2 by using our suggested route!")
print("------------------################--------------------------")

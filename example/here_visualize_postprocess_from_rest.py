...
This can act as a rest api which will return geojson data for available options.
...
if PROCESS_BIKE_STATION:
    BIKE_STATION = pd.read_excel(
        "../../Data_Blore/BIKE_STATION/BIKE_STATION Jan 2020.xlsx",
        "Sheet1",
        nrows=100,
    )
    ...
    for j in range(0, len(BIKE_STATION)):
        BIKE_STATION.append(
            (
                BIKE_STATION["start latitude"][j],
                BIKE_STATION["start longitude"][j],
            )
        )
